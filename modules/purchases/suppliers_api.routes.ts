...
router.get("/api/suppliers-detailed-report", async (req, res) => {
  try {
    ...
    const [itemsRes, supplierItemsRes] = await Promise.all([
      ...
      pool.query(supplierItemsSummaryQuery, params)
    ]);
    ...
    for (const r of rows) {
      totalItemsQuantity += Number(r.quantity || 0);
      totalItemsPrice += Number(r.total_price || 0);
    ...
    uniqueInvoices.forEach(inv => {
      totalInvoicesAmount += inv.total_amount;
      totalPaidAmount += inv.paid_amount;
      totalRemainingAmount += inv.remaining_amount;
    ...
    res.json({
      filters: { from, to, supplier_id, supplier_name, invoice_number, search },
      kpis: {
        ...
        invoices_count: uniqueInvoices.size,
    ...
    for (const item of items.rows) {
      ...
      if (purchase.warehouse_id && ingId) {
        const stockRes = await pool.query(
          `SELECT quantity, available FROM inventory_items WHERE warehouse_id = $1 AND ingredient_id = $2`,
          [purchase.warehouse_id, ingId]
        );
        if (stockRes.rows.length > 0) {
          stockQty = Number(stockRes.rows[0].quantity || 0);
          availableQty = Number(stockRes.rows[0].available ?? stockRes.rows[0].quantity ?? 0);
        }
      }
      enrichedItems.push({
        ...item,
...
router.post(["/api/returns/validate-stock", "/api/purchase-returns/validate-stock"], async (req: Request, res: Response) => {
  try {
    ...
    for (const item of returnItems) {
      const stockRes = await pool.query(
        `SELECT id, quantity, available FROM inventory_items WHERE warehouse_id = $1 AND ingredient_id = $2`,
        [whId, item.ingredient_id]
      );
      const availableQty = stockRes.rows.length > 0 
        ? Number(stockRes.rows[0].available ?? stockRes.rows[0].quantity ?? 0) 
        : 0;
  ...
  try {
    ...
    if (isApproved) {
      ...
      for (const item of returnItems) {
        ...
        const availableQty = Number(stockRow.available ?? stockRow.quantity ?? 0);
