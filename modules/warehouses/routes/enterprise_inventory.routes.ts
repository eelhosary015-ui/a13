...
import { recordGoodsReceiptCost } from "../../costs/services/cost.integration.service.js";
  ...
  try {
    ...
    if (Boolean(auto_post) && status === "approved") {
      ...
      for (const [ingredientId, requiredQty] of requiredByIngredient.entries()) {
        ...
        const available = Number(stockRes.rows[0]?.available ?? stockRes.rows[0]?.quantity ?? 0);
        if (!stockRes.rows.length || available < requiredQty) {
          await client.query("ROLLBACK");
          return res.status(400).json({
            success: false,
            error: `الرصيد غير كافٍ للصنف #${ingredientId}. المتاح: ${available}، المطلوب: ${requiredQty}`
          });
        }
      }

      // Move the approved quantities from source to destination in the same DB transaction.
      for (const item of formattedItems) {
        const qty = Number(item.requested_qty || 0);
